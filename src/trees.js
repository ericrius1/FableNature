...
// Deterministic RNG so the forest layout is stable between reloads.
...
// Tapered, slightly bent cylinder from `base` along `dir`.
function limbGeometry(base, dir, length, rBase, rTip, bend, rng) {
...
// Builds one tree at `origin`; pushes bark geometry into `barkGeos`
// and canopy cluster centers into `clusters`.
function buildOak(origin, scale, rng, barkGeos, clusters) {
  ...
  clusters.push({ center: top.clone().add(new THREE.Vector3((rng() - 0.5) * 2.4 * scale, -0.6 * scale, (rng() - 0.5) * 2.4 * scale)), r: (1.6 + rng() * 1.0) * scale });
}

function buildBirch(origin, scale, rng, barkGeos, clusters) {
  ...
  clusters.push({ center: top.clone().add(new THREE.Vector3((rng() - 0.5) * 1.6, -1.1 * scale, (rng() - 0.5) * 1.6)), r: (0.9 + rng() * 0.6) * scale });
}

// ---------- materials ----------

function oakBarkMaterial() {
...
function leafMaterial() {
  ...
  // Procedural leaf-cluster cutout: a rosette of elliptical leaflets.
  const p = uv().sub(0.5);
  let mask = float(0);
  const N = 7;
  for (let i = 0; i < N; i++) {
    ...
    const lx = dx.mul(ca).add(dy.mul(sa));     // along leaflet axis
    const ly = dx.mul(-sa).add(dy.mul(ca));    // across leaflet
  ...
  // Per-cluster + per-card tint variation; darker toward the card center.
  const h = hash(instanceIndex);
  const clusterTint = mx_noise_float(positionWorld.mul(0.35)).mul(0.5).add(0.5);
  let leafCol = mix(color(0x243d14), color(0x6a8a36), h.mul(0.5).add(clusterTint.mul(0.5)));
  leafCol = leafCol.mul(mix(float(0.7), float(1.18), smoothstep(0.0, 0.45, p.length())));
...
export function createForest() {
  ...
  // Scatter trees with a minimum spacing; keep a viewing corridor near camera.
  const placed = [];
  const tryPlace = (minR, maxR, minDist) => {
    for (let attempt = 0; attempt < 40; attempt++) {
      const a = rng() * Math.PI * 2;
      const r = minR + rng() * (maxR - minR);
      const x = Math.cos(a) * r, z = Math.sin(a) * r;
      if (z > 12 && Math.abs(x) < 6) continue; // don't block the camera
  ...
  // Hero oak near the camera, like the big tree in the reference.
  buildOak(new THREE.Vector3(7, terrainHeight(7, 2) - 0.3, 2), 1.5, rng, oakBark, clusters);
  placed.push({ x: 7, z: 2 });
  ...
  for (const c of clusters) {
    for (let i = 0; i < cardsPerCluster; i++) {
      // random point in canopy ellipsoid
      const a = rng() * Math.PI * 2;
      const u = rng() * 2 - 1;
